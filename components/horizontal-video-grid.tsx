...
import { Play, Lock, Star, Shield, ChevronLeft, ChevronRight } from "lucide-react"
...
interface HorizontalVideoGridProps {
  videos: Video[]
  title?: string
}

export default function HorizontalVideoGrid({ videos, title }: HorizontalVideoGridProps) {
      ...
      <div className="relative group">
        ...
        {showLeftArrow && (
          <Button
            ...
            variant="ghost"
            size="icon"
            className="absolute left-0 top-1/2 -translate-y-1/2 z-10 h-12 w-12 rounded-full bg-zinc-900/80 border border-zinc-700/50 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 shadow-lg"
          >
        ...
        {showRightArrow && (
          <Button
            ...
            variant="ghost"
            size="icon"
            className="absolute right-0 top-1/2 -translate-y-1/2 z-10 h-12 w-12 rounded-full bg-zinc-900/80 border border-zinc-700/50 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 shadow-lg"
          >
        ...
        >
          {videos.map((video) => (
            <Link
              key={video.id}
              href={`/video/${video.id}`}
              className="group relative overflow-hidden rounded-xl transition-all duration-300 hover:scale-[1.02] hover:shadow-xl hover:shadow-cyan-500/10 flex-shrink-0 w-[280px] snap-start"
            >
              {/* Card with 3D effect */}
              <div
